...
class QueryEnum(Enum):
  """
  QueryEnum is an Enum which you can construct conveniently by writing a string
  query. For example, for an enum Fruit with items Fruit.APPLE and Fruit.BANANA,
  you may wish to construct via Fruit.from_str("apple"), or even
  Fruit.from_str("a")...
  """
  @classmethod
  def from_str(cls, query: str):
    ...
    if len(matches) == 1:
      ...
      raise UnknownItem(
        ...
        f"Expected something resembling one of {[item.value for item in cls]}."
      )
