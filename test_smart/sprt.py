...
class UnknownDistributionError(ValueError):
  def __init__(self):
    super().__init__(
      f"Unknown Distribution. Expected one of {[d.value for d in Distribution]}"
    )
...
class SPRT(SeqHypothesisTest):
  ...
  for the Exponential Normal location parameters. When testing the Normal mean,
  you may optionally specify the standard deviation `sigma` during construction.
  If left unspecified, it is assumed to be 1.
  """
  ...
  def llr(self, x: np.ndarray) -> np.ndarray:
    ...
    """
    match self.dist:
      case Distribution.EXPONENTIAL: return self.exponential_llr(x)
      case Distribution.NORMAL: return self.normal_llr(x)
